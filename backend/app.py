...
def photo_rows(keyword: str | None = None) -> list[dict[str, Any]]:
    ...
    if keyword is not None:
        return query_all(base_sql + " WHERE p.keywords LIKE ? ORDER BY p.created_at DESC", (f"%{keyword}%",))
    return query_all(base_sql + " ORDER BY p.created_at DESC")
...
def reply_message(message_id: int) -> ResponseReturnValue:
    original = query_one(
        "SELECT id, sender_id, receiver_id, photo_id FROM messages WHERE id = ? AND receiver_id = ?",
...
def delete_message(message_id: int) -> ResponseReturnValue:
    ...
    execute("DELETE FROM messages WHERE id = ? AND receiver_id = ?", (message_id, session["user_id"]))
